...
const Amdibell = require('amdibell-public-2');
...
const AmdibellCon = new WAConnection();
const Session = new StringSession();
AmdibellCon.version = [2, 2126, 14]
setInterval(async () => { 
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (infoen !== '' && config.LANG == 'EN' ) {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return AmdibellCon.sendMessage(AmdibellCon.user.jid, '[ ```🔔Amdibell Announcements🔔``` ]\n\n' + infoen.replace('{user}', AmdibellCon.user.name).replace('{wa_version}', AmdibellCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', AmdibellCon.user.phone.os_version).replace('{device_model}', AmdibellCon.user.phone.device_model).replace('{device_brand}', QueenAmdiCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (infosi !== '' && config.LANG == 'SI') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return AmdibellCon.sendMessage(AmdibellCon.user.jid, '[ ```🔔Amdibell නිවේදන🔔``` ]\n\n' + infosi.replace('{user}', AmdibellCon.user.name).replace('{wa_version}', AmdibellCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', AmdibellCon.user.phone.os_version).replace('{device_model}', AmdibellCon.user.phone.device_model).replace('{device_brand}', AmdibellCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
}, 50000);
...
setInterval(async () => { 
    if (config.AUTO_BIO == 'true') {
        var tz_bio = await QueenAmdi.timezone(AmdibellCon.user.jid)
        var date = await QueenAmdi.datebio(config.LANG)
        const biography = '📅 ' + date + '\n⌚ ' + tz_bio + '    🎖️ ' + config.CAP
        await AmdibellCon.setStatus(biography)
    }
}, 7890);


// --------------Amdibell verify system--------------
        ...
        await axios.get('https://gist.githubusercontent.com/BlackAmda/c3877acdcdc041d77907d590d4ac1a2d/raw/').then(async (ann) => {
            ...
            if (verify !== '' && config.LANG == 'EN') {
                while (getGMTh == 20 && getGMTm == 55) { 
                    return AmdibellCon.sendMessage(AmdibellCon.user.jid, HANDLER + verify.replace('{user}', AmdibellCon.user.name).replace('{wa_version}', AmdibellConv.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', AmdibellCon.user.phone.os_version).replace('{device_model}', AmdibellCon.user.phone.device_model).replace('{device_brand}', AmdibellCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (verify !== '' && config.LANG == 'SI') {
                while (getGMTh == 20 && getGMTm == 55) { 
                    return AmdibellCon.sendMessage(AmdibellCon.user.jid, HANDLER + verify.replace('{user}', AmdibellCon.user.name).replace('{wa_version}', AmdibellCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', AmdibellCon.user.phone.os_version).replace('{device_model}', AmdibellCon.user.phone.device_model).replace('{device_brand}', AmdibellCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
    }, 50000);
...
// ======================Amdi_bell_Logger=======================
AmdibellCon.logger.level = config.DEBUG ? 'debug' : 'warn';
var nodb;
    ...
    if (StrSes_Db.length < 1) {
        ...
        AmdibellCon.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }

if (config.LANG == 'EN' ) {
    AmdibellCon.on ('credentials-updated', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login details updated!')
        );

        const authInfo = AmdibellCon.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
...
if (config.LANG == 'EN' ) {
    AmdibellCon.on('connecting', async () => {
...
if (config.LANG == 'EN' ) {
    AmdibellCon.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('🛡️ Amdibell Sheild Activated!')
        );
        await new Promise(r => setTimeout(r, 1100));

if (config.WORKTYPE == 'public') {
    if (AmdibellCon.user.jid === '@s.whatsapp.net') {

        await AmdibellCon.sendMessage(AmdibellCon.user.jid, '```🛡️ Blacklist Detected!``` \n```User:```  \n```Reason:``` ', MessageType.text)
    ...
    if (AmdibellCon.user.jid === '@s.whatsapp.net') {
        ...
        await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*Amdibell started in Private Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Private Mode. To change, use_ ```.setvar WORK_TYPE=public``` _command._\n\n *Thank you for using Amdibell 💌*', MessageType.text);
    }
}
    else {
    ...
    AmdibellCon.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
                ...
                if (AmdibellCon.user.jid === '@s.whatsapp.net') {
                    ...
                    await AmdibellCon.sendMessage(AmdibellCon.user.jid, '```🛡️ Blacklist අනාවරණය විය!``` \n```පරිශීලක:``` \n```හේතුව:``` ', MessageType.text)
                ...
                await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*Amdibell private ආකාරයට ක්‍රියාකිරිම ආරම්භ විය.👸*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n_ඔබේ command list එක ලබාගැනීමට_ *.panel* _command එක භාවිතා කරන්න._\n\n_ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට_ ```.setvar WORK_TYPE=public``` _විධානය භාවිතා කරන්න._\n\n *Amdibell භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
            }
...
AmdibellCon.on('message-new', async msg => {
        if (msg.key && msg.key.remoteJid == 'status@broadcast') return;

        if (config.NO_ONLINE) {
            await AmdibellCon.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        ...
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                await AmdibellCon.sendMessage(msg.key.remoteJid, Buffer.from (blogo.data), MessageType.image, {mimetype: Mimetype.png, caption: gb.message});
            }
            return;
            ...
            if (gb !== false) {
                await AmdibellCon.sendMessage(msg.key.remoteJid, Buffer.from (wlogo.data), MessageType.image, {mimetype: Mimetype.png, caption: gb.message});
            }
            return;
        }
        // ====================================================================
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = AmdibellCon.chats.get(msg.key.remoteJid)
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await AmdibellCon.chatRead(msg.key.remoteJid);
                        }
                        ...
                        && msg.message.imageMessage !== null) {
                            whats = new Image(AmdibellCon, msg);
                        } else if (command.on !== undefined && (command.on === 'video' )
                        && msg.message.videoMessage !== null) {
                            whats = new Video(AmdibellCon, msg);
                        } else {
                            whats = new Message(AmdibellCon, msg);
                        }
                        if (msg.key.fromMe) {
                            var vers = AmdibellCon.user.phone.wa_version.split('.')[2]
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*-- දෝෂ වාර්තාව [AMDI_BELL] --*' + 
                                ...
                                await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*-- ERROR REPORT [AMDI_BELL] --*' + 
                                ...
                                if (error.message.includes('URL')) {
                                    return await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*⚜️ ERROR ANALYSIS [AMDI_BELL] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*⚜️ ERROR ANALYSIS [AMDI_BELL] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                        '\n\n*Main Error:* _Ookla Server AmdibellConection_' +
                                    ...
                                    return await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*⚜️ ERROR ANALYSIS [AMDI_BELL] ⚜️*' + 
                                        ...
                                        '\n\n*Reason:* _AmdibellConection between site disAmdibellConected._' +
                                    ...
                                    return await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*⚜️ ERROR ANALYSIS [AMDI_BELL] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                        '\n\n*Main Error:* _Media downloading error_' +
                                        '\n\n*Reason:* AmdibellConection between site disAmdibellConected._' +
                                        '\n\n*Solution:* _Wait for few minutes. This error will fixed By Developers._'
                                        , MessageType.text
                                    );
                                }
                                    ...
                                    return await AmdibellCon.sendMessage(AmdibellCon.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
